...
class ConstituentNetFourier(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.out_layer = nn.Sequential(
            nn.LayerNorm(embbed_dim), nn.Linear(embbed_dim, num_classes)
        )
        self.cls_token = nn.Parameter(
            torch.randn(1, 1, embbed_dim)
        )  # learned classification token, (1, 1, C)
    ...
    def forward(self, x):
        ...
        out = self.out_layer(out).squeeze(
            1
        )  # (m_batch, 1, C) -> (batch_m, num_classes)
...
class ConstituentNetCluster(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder_blocks = nn.ModuleList(
            [
                Transformer(embbed_dim, num_heads=num_heads, dropout=dropout)
                for _ in range(num_transformers)
            ]
        )
        self.norm = nn.LayerNorm(embbed_dim)
        self.decoder_blocks = nn.ModuleList(
            [
                CrossTransformer(embbed_dim, num_heads=num_heads, dropout=dropout)
                for _ in range(num_transformers)
            ]
        )
        self.out_layer = nn.Sequential(
        ...
        self.clusters = nn.Parameter(
            torch.randn(1, num_clusters, embbed_dim)
        )  # learned clusters (1, num_clusters, C)
    ...
    def forward(self, x):
        ...
        out = self.out_layer(
            out.view(m_batch, -1)
        )  # (m_batch, num_clusters, C) -> (m_batch, num_classes)
...
class ConstituentNetPerceiver(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.embbeddings = nn.Parameter(
            torch.randn(1, num_embbeds, embbed_dim)
        )  # learned embbeddings (1, num_embbeddings, C)
    ...
    def forward(self, x):
        ...
        out = self.out_layer(
            out.view(m_batch, -1)
        )  # (m_batch, num_embbeddings, C) -> (m_batch, num_classes)
...
class ConstituentNetPerceiverRecurrent(nn.Module):
    ...
    def __init__(
        ...
        num_embbeds: int = 4,
        num_classes: int = 5,
        num_perceivers: int = 5,
        num_latent_transformers: int = 2,
        num_heads: int = 1,
        ...
        **kwargs
        ...
        self.query_hidden = nn.Parameter(
            torch.randn(1, 1, embbed_dim)
        )  # initial query hidden state (1, 1, C)
    ...
    def forward(self, x):
        ...
        # Perceivers
        for perceiver in self.perceivers:
            out, energy, query_hidden = self.perceiver(
                out, x, query_hidden
            )  # (m_batch, num_embbeddings, C)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.graph_assign = nn.Sequential(
            nn.Linear(in_dim, 64), nn.Softmax(dim=-1)  # number of graph nodes
        )
        ...
        self.out_layer = nn.Sequential(nn.Linear(256 * 2, num_classes))
...
def accuracy_top_k(
    outputs: torch.Tensor, targets: torch.Tensor, topk: int = 5
) -> float:
    """
