...
class Attention(nn.Module):
    ...
    def __init__(
        self,
        in_dim: int,
        context_dim: typing.Optional[int] = None,
        ...
        assert (
            ...
            "Input dim needs to be divisible by num_heads, since out_dim == in_dim."
        )
        assert (
    ...
    def forward(self, queries, keys, values):
        ...
        queries = self.queries(queries).view(
            m_batch, query_seq_len, self.heads, C_H
        )  # (batch_m, query_seq_len, num_heads, C_head)
        keys = self.keys(keys).view(
            m_batch, key_seq_len, self.heads, C_H
        )  # (batch_m, key_seq_len, num_heads, C_head)
        values = self.values(values).view(
            m_batch, key_seq_len, self.heads, C // self.heads
        )  # (batch_m, key_seq_len, num_heads, C // num_heads)

        # Attention softmax(Q^T*K)
        energy = torch.einsum(
            "nqhc,nkhc->nhqk", [queries, keys]
        )  # (batch_m, num_heads, query_seq_len, key_seq_len)
        ...
        # Output
        out = torch.einsum(
            "nhql,nlhc->nqhc", [attention, values]
        )  # (batch_m, query_seq_len, num_heads, C // num_heads)
...
class AttentionDynamicQuery(nn.Module):
    ...
    def forward(self, queries, keys, values, queries_hidden):
        ...
        queries = self.queries(
            queries.view(m_batch * query_seq_len, C),
            queries_hidden.view(m_batch * query_seq_len, C),
        )  # (batch_m * query_seq_len, C)
        queries = queries_hidden = queries.view(
            m_batch, query_seq_len, self.heads, C_H
        )  # (batch_m, query_seq_len, num_heads, C_head)
        keys = self.keys(keys).view(
            m_batch, key_seq_len, self.heads, C_H
...
class CrossTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = (
            SelfAttention(in_dim, latent_dim=self.latent_dim, num_heads=num_heads)
            if self.use_self_attention
            else nn.Identity()
        )
        self.cross_attention = Attention(
            in_dim, context_dim=self.context_dim, num_heads=num_heads
        )
    ...
    def forward(self, queries, contexts):
        ...
        queries, energy = self.cross_attention(
            queries, contexts, contexts
        )  # cluster-constituent attention
...
class CrossTransformerDynamicQuery(nn.Module):
    ...
    def __init__(
        ...
        self.cross_attention = AttentionDynamicQuery(
            in_dim, context_dim=self.context_dim, num_heads=num_heads
        )
        self.linear = nn.Sequential(
            nn.LayerNorm(in_dim),
    ...
    def forward(self, queries, contexts, queries_hidden):
        ...
        queries = self.self_attention(queries)  # inter-cluster self-attention
        queries, energy, queries_hidden = self.cross_attention(
            queries, contexts, contexts, queries_hidden
        )  # cluster-constituent attention
        out = self.linear(queries)
        out = queries + out  # res-connection
        out = self.dropout(out)
    ...
    def __init__(
        ...
        self.transformers = nn.ModuleList(
            [
                ...
                for _ in range(num_latent_transformers)
            ]
        )
...
class PerceiverDynamicQuery(nn.Module):
    ...
    def __init__(
        ...
        self.cross_transformer = CrossTransformerDynamicQuery(
            in_dim,
            context_dim=context_dim,
            latent_dim=self.latent_dim,
            num_heads=num_heads,
    ...
    def forward(self, x, constituents, queries_hidden):
        ...
        # Query - cross-attention latent and constituents input spaces
        out, energy, queries_hidden = self.cross_transformer(
            x, constituents, queries_hidden
        )
